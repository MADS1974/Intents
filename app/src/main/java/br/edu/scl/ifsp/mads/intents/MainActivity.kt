package br.edu.scl.ifsp.mads.intents
...
import br.edu.scl.ifsp.mads.intents.R
import br.edu.scl.ifsp.mads.intents.databinding.ActivityMainBinding
...
class MainActivity : AppCompatActivity() {
            ...
            with (result) {
                if (resultCode == RESULT_OK) {
                    data?.data?.also {
                        ...
                        startActivity(Intent(ACTION_VIEW).apply { setData(it) })
                    }
                }
            }
        }
            ...
            R.id.chooserMi -> {
                startActivity(
                    Intent(ACTION_CHOOSER).apply {
                        putExtra(EXTRA_TITLE, getString(R.string.chose_your_favorite_browser))
    ...
    private fun callPhone(call: Boolean) {
        startActivity(
            Intent(if (call) ACTION_CALL else ACTION_DIAL).apply {val phoneNumber = "tel: ${activityMainBinding.parameterTv.text}"
                this.data = Uri.parse(phoneNumber) // Correct: calling on the Intent instance
            }
        )
    }
